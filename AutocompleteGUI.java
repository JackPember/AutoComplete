...
//            System.out.println(Arrays.toString(test));
    ...
    /**
     * Ask the user to select a database file in JOptionPane.
     * @return the string file name of the selected database.
     */
    private static String pickFile() {
    	List<String> databaseOptions = new ArrayList<>();
    	File inputsFolder = new File("InputDatabaseFiles");
    	for (File file : inputsFolder.listFiles()) {
			databaseOptions.add(file.getName());
		}
    	String choice = (String) JOptionPane.showInputDialog(null, "What database do you want to use?", "Choose Database", JOptionPane.QUESTION_MESSAGE, null, databaseOptions.toArray(), null);
    	if (choice == null) {throw new IllegalArgumentException("File Not Selected!");}
    	
    	return inputsFolder.getAbsolutePath() + "\\" + choice;
    }
    
    /**
     * Ask user how many suggestions they want for their searches.
     * @return the number of suggestions.
     */
    private static int numberOfSuggestions() {
    	String input = (String) JOptionPane.showInputDialog("How many suggestions would you like for each search?");
    	int suggestNum;
    	try {
			suggestNum = Integer.valueOf(input);
		} catch (Exception e) {
			throw new IllegalArgumentException("Input was not an integer number!");
		}
    	return suggestNum;
    }
    ...
    public static void main(String[] args) {
    	...
    	String filename;
    	int suggestions = 5; // Default
    	
    	// Ask user for database and number of suggestions:
        ...
        if (argsLength > 0) {
        	...
        	filename = args[0];
        	if (argsLength != 1) {
        		suggestions = Integer.parseInt(args[1]);
			}
        } else {
			filename = pickFile();	
			suggestions = numberOfSuggestions();
		}
        final int suggestNum = suggestions;
        
        
        // Start GUI and AutoComplete Stuff:
        SwingUtilities.invokeLater(
                new Runnable() {
                    public void run() {
                        new AutocompleteGUI(filename, suggestNum).setVisible(true);
                    }
                });
    }
}